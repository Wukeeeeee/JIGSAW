...
- **工业级正交曼哈顿折线（Orthogonal Manhattan Routing）**：
  ...
  - **交互支持**：支持连线点击选中高亮、独立正交箭头 markers 标记、正交连线中央悬浮快捷删除按键，支持 <kbd>Delete</kbd> 快捷键删除选中连线或节点。
- **控制流与逻辑门节点体系**：
  - 新增「逻辑控制」与「智能体」分类选项卡；
...
- **工具系统（内置 12 个工具）**：
  ...
  - 知识库：`knowledge_search`（关键词切词检索原文片段）/ `knowledge_info`（查询结构）/ `knowledge_write`（落盘新建/追加/覆盖写入）
...
### 4. 本地知识库与项目工作空间
- **本地知识库**：三栏文件树结构，支持子目录与嵌套分类；支持 Word / PDF / Excel / PPT 等多种文件导入；系统级隐藏文件（`.DS_Store` / `.git` 等）统一自动过滤；AI 侧工具支持精确检索与落盘读写。

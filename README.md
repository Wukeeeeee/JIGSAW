...
**当前状态**：前端 UI + FastAPI 后端 + 真实 LLM 链路（OpenAI 兼容接口，多模型并存，密钥全程不出后端）+ 工具调用（含通用生图与代码制图展示，16 个内置工具，含代码文件阅读三件套）+ 权限控制 + 异步任务队列（多 Worker 并发、可中断，工作流节点执行与 Captain 规划均由后端驱动）+ 本地知识库（AI 可读写，RAG 向量语义混合检索）+ 思考链防抖稳定展示与大图灯箱 + 工具调用统计 + 曼哈顿正交布线与逻辑门工作流画布 + 桌面壳。
...
- **任务复杂度自适应动态伸缩（Dynamic Complexity Scaling）**：
  ...
  - **Tier 2 中度专项任务**（单一主题调研、产品对比、创意海报）：自动匹配为 3~5 个节点（2~3路并行前置探索 ➔ 终极总成交付）；
  - **Tier 3 复杂长文本/重型课题**（长篇需求、多维度商业规划、跨界战略推演）：自动展开为 6~10 节点、3~5 阶纵深流水线。
- **AI 自主生成逻辑控制拓扑（Condition / Loop）**：
  - Captain 规划可输出**条件分支节点**（`type: condition`，True/False 双路分流，如“资料充分 ➔ 直接深析 / 不足 ➔ 补充调研后汇聚”）与**循环节点**（`type: loop`，草稿 ⇄ 评审最多 N 轮回环，达标后放行终稿）；
  - 条件判定由后端 LLM 基于上游真实输出作出（只回 true/false），不再是固定预设，判定失败如实熔断下游分支；
  - 用户在任务中明确提出“复杂 / 深度 / 多阶段 / 系统性”时，Captain 自动展开为 8~15 节点、4~6 阶、含分支与循环的完整拓扑；
  - 规划输出带截断保护：大图 JSON 被模型输出上限截断时明确报错，绝不静默降级为简化的模板图。
- **后端驱动执行引擎（Backend-Driven Execution）**：工作流节点执行与 Captain 规划均由后端任务队列驱动（可取消、经权限门控、AskUser 可挂起提问），浏览器只负责画布渲染与编排控制，API Key 全程不出后端；画布拓扑与节点状态实时落盘 `backend/data/workflows.json`，刷新/重启不丢。
...
- **全局规则（Rules）**：设置 → 常规 → 全局规则，写给 AI 的持久偏好（语言/风格/项目约定），每次对话与工作流节点自动注入系统提示词，保存即生效无需重启；落盘 `backend/data/rules.md`（超 4000 字自动截断保护上下文）。
...
- **工具系统（内置 16 个工具）**：
  ...
  - 代码文件三件套（AI Coding 基础）：`read_file`（读取文本文件，带行号，支持 offset/limit 分页）/ `list_files`（树状列出目录结构与文件大小，自动忽略 node_modules/.git 等）/ `grep`（正则搜索文件内容，返回 文件:行号:匹配行）
  - 图像：通用图像生成（`generate_image`，支持主流服务商与画幅定制）
  - 文件：文件内容读取（`read_extra` 统一提取层，覆盖 Word / PDF / Excel / PPT）/ 文件编辑 / 应用代码补丁（apply_patch）
...
- **本地安全防线**：
  - **Origin 守卫**：所有写请求（非 GET）来自非本机 Origin 直接 403 —— 恶意网页无法驱动本地后端（含"替用户回答权限确认弹窗"的攻击链）；CORS 收敛为本机白名单；
  - **密钥出清**：`/api/models` 与 `/api/settings` 一律脱敏（只回 `hasKey` 标记，不回密钥明文），对话/节点执行/规划只传 `model_id` 由后端解析——**API Key 不出现在任何 HTTP 请求与响应里**；编辑模型时密钥留空 = 保留原密钥，清浏览器缓存不再丢密钥；
  - **高危工具门控**：`shell` / 文件写入 / 覆盖知识库等只能由 AI 在任务链路中经权限确认后调用，通用执行接口直接拒绝。
